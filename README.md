...
- a guarded free companion that works out of the box
...
- use `CHAT_PROVIDER=offline` for a free, built-in guarded companion
...
- `CHAT_PROVIDER` default is `offline`
...
In offline mode, the server uses a guarded local response engine. In OpenAI mode, the server sends fixed WitnessReady instructions on every request.
